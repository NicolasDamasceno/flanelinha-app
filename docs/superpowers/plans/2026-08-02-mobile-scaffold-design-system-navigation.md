...
**Testing approach:** No automated test framework (same decision as the backend sub-project — this is a learning project with no CI). Verification is `npx tsc --noEmit` for every task. For tasks that add or change files under `mobile/app/` (new routes), regenerate the typed-routes definitions **first**: delete `mobile/.expo/types` (`rm -rf .expo/types`, or PowerShell `Remove-Item -Recurse -Force .expo/types -ErrorAction SilentlyContinue`), then run `npx expo customize tsconfig.json` from `mobile/`. `npx expo export` does NOT regenerate this file at all in the currently-installed `@expo/cli` version (57.0.11) — only `expo start`'s dev server does, and repeatedly starting/stopping a dev server just to refresh types is unnecessarily slow and fragile for a non-interactive task loop. `.expo/` is gitignored, so none of this ever touches a commit.

**Root-caused environment hazard (read this if you ever see route paths like `/../src/components/X` in a `tsc` error):** this corruption happened twice across this plan's execution, and both times traced back to the same cause — a **lingering `npx expo start` background process** (started by an earlier task as a workaround before this `expo customize` recipe was established) that was never actually terminated. Its file watcher kept re-writing `.expo/types/router.d.ts` with bogus entries every time a new file was added under `mobile/src/`, silently overwriting whatever clean file `expo customize` had just produced. **If you ever run `npx expo start` for any reason (in this project or any other Expo project), verify it is actually killed afterward** — e.g. `Get-Process node | Where-Object { $_.CommandLine -match 'expo' }` in PowerShell, or `ps aux | grep 'expo.*start'` in bash — don't just assume Ctrl+C or closing a terminal tab killed it. This plan's recipe (`rm -rf .expo/types` + `expo customize tsconfig.json`) never needs `expo start` at all, so this shouldn't come up again if you stick to it — but if a stray process from outside this plan's own steps is still running, no amount of deleting-and-regenerating will stay fixed for long.

Task 11 (the last code task) adds one real `npx expo export --platform android` as an end-to-end bundling sanity check; the final task (12) adds real manual testing against the running backend via Expo Go/emulator (which legitimately needs `expo start` running — that's fine, it's the intended long-lived use).
